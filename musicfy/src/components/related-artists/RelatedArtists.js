            ...
            <div>
                ...
                <ul className="related-artist">
                    {relatedAlbums.map((related) => {
                        ...
                        return (
                            <li className="music-item" onMouseEnter={(event) => setEventOnMouseMove(event, related.name)} onMouseLeave={(event) => setEventOnMouseMove(event, related.name)}>
                                <div style={backgroundMusicImage}></div>
                                <div className="description-container">
                                    <span>{related.name}</span>
                                    <div className="playbutton-container"><span>{related.from}</span><PlayButton mouseIsOver={mouseIsOver.key === related.name && mouseIsOver.isOver === true} clickHandler={() => { pickArtist(related) }} /></div>
                                </div>
                            </li>
                        );
                    })}
